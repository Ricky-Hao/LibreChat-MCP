...
- v0.2.0 revision: JSON config uses `baseUrl` with literal `refreshToken` as the only upstream credential, no old jwt configuration compatibility. For LibreChat provider mode, obtain an in-memory JWT on first API call and refresh/replay once on explicit 401. Singleflight per client/process, persist rotated refresh cookie atomically into the same config, including after a malformed JWT body. Graceful shutdown drains active refresh/save. No username/password login, OIDC reuse flow or secret-file indirection.
